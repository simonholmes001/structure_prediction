        ...
        for root, dirs, files in os.walk('./' + self.walk_path, topdown=False):
            for name in tqdm(files):
                if 'adjacency' in name:
                    ...
                    merge_features_and_adjacency = pd.merge(merge_amino_acids_and_features, adjacency_matrix_df, on=0, how='left')  # Join the databases
                    merge_features_and_adjacency.to_csv('./' + self.walk_path + '/FINAL_' + name, encoding='utf-8', index=False, header=False)

def main():
    merge = MergeAAFeatures(args.output_directory, df_amino_acids, df_standardised_features)
